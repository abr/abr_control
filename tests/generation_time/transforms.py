"""
For benchmarking the generation time of the transforms Tx
...
for ii in range(100):
    ...
    for config in configs:
        T.append(config.T(test_points[-1], q=q))
    for ii in range(len(configs) - 1):
        assert np.allclose(T[ii].flatten(),
                           T[ii+1].flatten(),
                           atol=1e-7)
...
plt.savefig('results/transform.pdf')
plt.show()
