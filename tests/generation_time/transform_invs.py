"""
For benchmarking the generation time of the transform inverse T_inv
...
test_points = ['joint%i' % ii for ii in range(configs[0].num_joints)]
...
for ii, joint in enumerate(test_points):
    for jj, config in enumerate(configs):
        ...
        T_inv = config.T_inv(joint, q=q)
        gen_times[ii, jj] = time.time() - start_time
        print('%s Tx gen time: %.5f' %
...
for ii in range(100):
    ...
    for config in configs:
        T_inv.append(config.T_inv(test_points[-1], q=q))
    for ii in range(len(configs) - 1):
        assert np.allclose(T_inv[ii].flatten(),
                           T_inv[ii+1].flatten(),
                           atol=1e-7)
...
for ii, joint in enumerate(test_points):
    for jj, config in enumerate(configs):
        ...
        print('Average Tx time: %.5f' % run_times[ii, jj])
    print("")
...
plt.savefig('results/transform_invs.png')
plt.savefig('results/transform_invs.pdf')
plt.show()
