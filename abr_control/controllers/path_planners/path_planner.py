...
class PathPlanner():
    ...
    def generate_path(
            self, start_position, target_position, max_velocity,
            start_orientation=None, target_orientation=None,
            start_velocity=0, target_velocity=0,
            plot=False):
        """
        ...
        for ii, t in enumerate(np.linspace(0, 1, self.n_sample_points)):
            ...
            warped_target = np.dot(R, (1/np.sqrt(3))*self.pos_profile.step(t)*dist) + start_position
        ...
        if isinstance(start_orientation, list) or isinstance(start_orientation, (np.ndarray, np.generic)):
            if isinstance(target_orientation, list) or isinstance(target_orientation, (np.ndarray, np.generic)):
                ...
                if self.verbose:
                    self.log.append(f"{c.blue}start_orientation={start_orientation} | target_orientation={target_orientation}{c.endc}")
            else:
                raise NotImplementedError (f"{c.red}A target orientation is required to generate a path{c.endc}")
            ...
            if plot:
                self._plot(start_position=start_position, target_position=target_position, ang=True)
        else:
            self.angular_planner = False
            self.path = np.hstack((self.position_path, self.velocity_path))
            if plot:
                self._plot(start_position=start_position, target_position=target_position, ang=False)
        ...
        if self.verbose:
            self.log.append(f"{c.blue}Time to converge: {self.time_to_converge}{c.endc}")
            self.log.append(f"{c.blue}dt: {self.dt}{c.endc}")
            self.log.append(f"{c.blue}pos x error: {self.position_path[-1, 0] - target_position[0]}{c.endc}")
            self.log.append(f"{c.blue}pos y error: {self.position_path[-1, 1] - target_position[1]}{c.endc}")
            self.log.append(f"{c.blue}pos x error: {self.position_path[-1, 2] - target_position[2]}{c.endc}")
            self.log.append(f"{c.blue}2norm error at target: {np.linalg.norm(self.position_path[-1] - target_position[:3])}{c.endc}")
    ...
    def _plot(self, start_position, target_position, ang=None):
        ...
        ax3.scatter(start_position[0], start_position[1], start_position[2], label='start')
        ...
        plt.hlines(start_position[0], 0, t[-1], linestyle='--', color='y', label='start')
        ...
        plt.hlines(start_position[1], 0, t[-1], linestyle='--', color='y', label='start')
        ...
        plt.hlines(start_position[2], 0, t[-1], linestyle='--', color='y', label='start')
