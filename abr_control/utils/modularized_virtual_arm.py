...
class VirtualArm():
    def __init__(animate=False, step=10, use_cache=True, db_name=None,
            n_columns=1, plot_extra=None, runs, sessions, test_groups,
            test_names):
        """
        Instantiates required modules and creates figure object

        PARAMETERS
        ----------
        animate: boolean, Optional(Default: False)
            True: create a figure skipping the number of frames set by 'step'
            to speed up calculation
            False: create a figure of the last frame
        step: int, Optional (Default: 10)
            How many data points to skip when creating an animated figure.
            This helps cut down on calculation time
        ...
        n_columns: int, Optional (Default: 1)
            If plotting multiple runs, select how many rows to break up the
            subplot into
        plot_extra: list of strings, Optional (Default: None)
            None: plot virtual arm data only
            Otherwise pass in a list of parameters to load from the database to
            plot onto a subplot adjacent to its corresponding virtual arm plot
            Ex: ['error', 'u_osc']
        runs: list of ints
            the list of test numbers to plot
        sessions: list of ints
            the corresponding sessions of the runs to be plotted
    ...
    def add_to_plot(show_trajectory=False):
        """
        Added provided data to plot
