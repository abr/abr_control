...
for ii in range(0,length,10):
    ...
    ax = fig.add_subplot(1,3,1, projection='3d')
    q = q_t[0][ii]
    targets = targets_t[0][ii]
    error = error_t[0][ii]
    ee_xyz = ee_xyz_t[0][ii]
    filter_xyz = filter_t[0][ii, 0:3]
    ...
    q = q_t[1][ii]
    targets = targets_t[1][ii]
    error = error_t[1][ii]
    ee_xyz = ee_xyz_t[1][ii]
    filter_xyz = filter_t[1][ii, 0:3]
    ...
    plt.title('%s \nDistance to Target: %.3f m'%(test_name, error))
    if show_traj:
        ax.plot(ee_xyz_t[1][:ii, 0], ee_xyz_t[1][:ii,1], ee_xyz_t[1][:ii,2],
                c='b')
        ax.plot(filter_t[1][:ii, 0], filter_t[1][:ii,1], filter_t[1][:ii,2],
                c='g')
    ...
    q = q_t[2][ii]
    targets = targets_t[2][ii]
    error = error_t[2][ii]
    ee_xyz = ee_xyz_t[2][ii]
    filter_xyz = filter_t[2][ii, 0:3]
    ...
    plt.title('%s \nDistance to Target: %.3f m'%(test_name, error))
    if show_traj:
        ax.plot(ee_xyz_t[2][:ii, 0], ee_xyz_t[2][:ii,1], ee_xyz_t[2][:ii,2],
                c='b')
        ax.plot(filter_t[2][:ii, 0], filter_t[2][:ii,1], filter_t[2][:ii,2],
                c='g')
    plt.tight_layout()
