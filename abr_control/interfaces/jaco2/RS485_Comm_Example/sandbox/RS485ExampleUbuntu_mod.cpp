...
// define an array for storing the time stamps
float timestamps[LOOPCOUNT];

struct timeval tv;

float dist = LOOPCOUNT * (40 * (0.0025));
float positions[LOOPCOUNT * 2];

int delay = 1750; //packet loss <1%, @ 1000us packet loss ~6%
...
int main()
{
	...
	if(fptrInitCommunication != NULL || MyRS485_Activate != NULL || MyRS485_Read != NULL || MyRS485_Write != NULL)
	{
		...
		if(result == NO_ERROR_KINOVA)
		{
			...
			TrajectoryMessage.SourceAddress = 0x00;
			TrajectoryMessage.DestinationAddress = 0x15;
			TrajectoryMessage.DataFloat[0] = 0x00000000;//Joint6Command; //32F position command [deg]
			TrajectoryMessage.DataFloat[1] = 0x00000001; //32F torque command [Nm]
			TrajectoryMessage.DataLong[2] = 0x00010001; //U16|U16
			TrajectoryMessage.DataLong[3] = 0x01; //U8|U8|U8|U8
			...
			for(int i = 0; i < LOOPCOUNT; i++)
			{
				...
				TrajectoryMessage.DataFloat[0] = Joint6Command;
				TrajectoryMessage.DataFloat[1] = 0x00000001;//Joint6Command;
            ...
            cout << (end_time_in_mill - start_time_in_mill) / (LOOPCOUNT) << "\n";
        }
	}
	else
	{
...
void* ReadPosition(void *ptr)
{
	...
	for(int i = 0; i < (LOOPCOUNT * 2); i++)
	{
		...
		for(int j = 0; j < MessageReadCount; j++)
		{
			...
			{
                positions[i] = MessageListIn[j].DataFloat[1];
    ...
    cout << "Packets: " << packet_count << " | expected: " << LOOPCOUNT << endl;
