...
for ii in range(0, 10):
    ...
    try:
        ...
        ctrlr = abr_control.controllers.osc_adaptive.controller(
            ...
            weights_file='data/4.2/weights%i.npz' % ((ii - 1) % save_history),
            encoders_file='data/4.2/encoders%i.npz' % ((ii - 1) % save_history))
        ...
        if count > 0:
            ...
            with open('data/4.2/total_error_track.txt', 'a') as myfile:
                myfile.write('%.3f\n' % np.sum(error_track))
            ...
            if plotting is True:
                ...
                plt.savefig('data/4.2/error%i' % ii)
