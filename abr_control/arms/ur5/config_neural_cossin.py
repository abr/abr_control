...
class UR5ConfigNeuralCos(config.UR5Config):
    ...
    def T(self, name, q, format_q=True):
        ...
        if format_q is True:
            q = self.format_q(q)
        return super(RobotConfig, self).T(name, q)
    ...
    def _calc_T(self, name, lambdify=True):
        ...
        Tx = super(RobotConfig, self)._calc_T(name=name, lambdify=False)
    ...
    def _calc_J(self, name, lambdify=True):
        ...
        """
        # get the transform using the cos and sin matrices defined above
        J = super(RobotConfig, self)._calc_J(name=name, lambdify=False)
    ...
    def _calc_Mq(self, lambdify=True):
        ...
        """
        # get the transform using the cos and sin matrices defined above
        Mq = super(RobotConfig, self)._calc_Mq(lambdify=False)
    ...
    def _calc_Mq_g(self, lambdify=True):
        ...
        """
        # get the transform using the cos and sin matrices defined above
        Mq_g = super(RobotConfig, self)._calc_Mq_g(lambdify=False)
