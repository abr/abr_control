"""
Minimal example using the mujoco interface with a two link arm
controlling two degree of freedom

To plot the joint angles call the script with True
passed through sys.argv
    python twolink.py True
"""
...
try:
    while True:
        ...
        if np.linalg.norm(hand_xyz[0:2] - target[0:2]) < 0.01:
            target[0] = np.random.uniform(0.1, 0.12) * np.sign(np.random.uniform(-1, 1))
            target[1] = np.random.uniform(0.1, 0.12) * np.sign(np.random.uniform(-1, 1))
            interface.set_mocap_xyz('target', target[:3])
