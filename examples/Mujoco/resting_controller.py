...
resting = RestingConfig(robot_config, rest_angles=rest_angles, kp=12)
...
try:
    ...
    while 1:
        ...
        u_rest = resting.generate(q=feedback["q"], dq=feedback["dq"])
    ...
    if ee_track.shape[0] > 0:
        ...
        for ii, q in enumerate(q_track):
            if rest_angles[ii] is not None:
                ...
                ax1.plot(
                    np.ones(len(q_track.T)) * rest_angles[ii], c=col[ii], linestyle="--"
                )
        plt.legend()
        plt.show()
