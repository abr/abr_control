...
for log, use_cython in zip(logs, [False, True]):
    for arm, config_params, osc_params in param_sets:
        log.append(test_timing(
            arm, config_params, osc_params, use_cython=use_cython))
...
plt.subplot(2, 1, 1)
python = plt.bar(np.arange(n), timing_tests_python, width=.5)
...
plt.subplot(2, 1, 2)
cython = plt.bar(np.arange(n), timing_tests_cython, width=.5, color='C1')
plt.ylabel('Time (ms)')
